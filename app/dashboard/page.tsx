...
export default function Dashboard() {
  ...
  useEffect(() => {
    const syncPoints = () => {
      const storedPoints = localStorage.getItem('neuro_points');
      if (storedPoints !== null) {
        const parsedPoints = Number(storedPoints);
        if (!Number.isNaN(parsedPoints)) {
          setStats((prev) => ({ ...prev, points: parsedPoints }));
        }
      }
    };
    ...
    window.addEventListener('focus', loadUserPoints);

    return () => {
      window.removeEventListener('storage', syncPoints);
      window.removeEventListener('focus', loadUserPoints);
    };
  }, [sessionStatus]);
  ...
  const loadUserPoints = async () => {
    try {
      ...
      const data = await response.json();
      const latestPoints = data?.user?.points || 0;
      localStorage.setItem('neuro_points', String(latestPoints));
      setStats(prev => ({
        ...prev,
        points: latestPoints,
      }));
